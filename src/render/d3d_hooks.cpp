...
// Translate a bound shader once (cache by guest address). Probes the bytes at the
// pointer first (we don't yet know if it's the raw Xenos container or a D3D9
// object), then attempts XenosRecomp -> DXIL and logs the outcome.
void TranslateShaderOnce(const char* tag, uint32_t guest_addr, const uint8_t* base) {
  if (guest_addr == 0) return;
  static std::unordered_set<uint32_t> done;
  if (!done.insert(guest_addr).second) return;

  const uint8_t* host = base + guest_addr;
  uint8_t hdr[32] = {};
  size_t got = mm::SafePeek(host, hdr, sizeof(hdr));
  char hex[sizeof(hdr) * 3 + 1] = {};
  for (size_t i = 0; i < got; ++i) std::snprintf(hex + i * 3, 4, "%02X ", hdr[i]);

  bool is_pixel = false;
  auto dxil = got ? mm::TranslateXenosShaderToDxil(host, ShaderInclude(), is_pixel)
                  : std::vector<uint8_t>{};
  ShaderTracef("[mm-shader] %s addr=0x%08X hdr=[%s] -> %s dxil=%zu %s\n", tag, guest_addr, hex,
               is_pixel ? "PIXEL" : "VERTEX", dxil.size(), dxil.empty() ? "FAIL" : "OK");
}
...
// Shader setters: capture the bound shader (r5 = shader object, from the trace)
// and translate it via XenosRecomp -> DXIL, caching + logging the result.
...
REX_HOOK_RAW(rex_D3DDevice_SetVertexShader) {
  ...
  TranslateShaderOnce("VS-r4", ctx.r4.u32, base);
  TranslateShaderOnce("VS-r5", ctx.r5.u32, base);
...
REX_HOOK_RAW(rex_D3DDevice_SetPixelShader) {
  ...
  TranslateShaderOnce("PS-r4", ctx.r4.u32, base);
  TranslateShaderOnce("PS-r5", ctx.r5.u32, base);
