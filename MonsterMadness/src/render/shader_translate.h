...
std::vector<uint8_t> TranslateXenosShaderToDxil(const uint8_t* shaderContainer,
                                                ...
                                                std::vector<uint32_t>* interpolatorSemanticsOut = nullptr);
...
// Translate a pixel shader's specialized variant ucode (from the CP thread) that matches NO
// container, by synthesizing a minimal container around it (empty constant table + an interpolator
// table built from the COMPANION VERTEX SHADER's exported interpolator semantics `vsInterp` —
// each entry (usage<<8)|usageIndex in slot order, from TranslateXenosShaderToDxil's
// interpolatorSemanticsOut). The PS reads slot i (register i) from the VS's slot-i semantic, which
// is how Xenos links interpolators VS->PS. If `vsInterp` is empty, falls back to a generic
// TexCoord{i} table. Returns DXIL or empty.
...
// If `attrsOut` is non-null it receives the recompiler's parsed attributes (usage/format/offset),
// so the caller can pick the template whose layout matches the authoritative vfetch layout
// (the SDK's vertex_bindings) rather than just the attribute COUNT — a count-only match can pick
// a template that mis-maps semantics and collapses the vertex position.
int ProbeSplicedVsAttrs(const uint8_t* templateContainer, const uint8_t* ucode,
                        ...
                        std::vector<VertexInputAttr>* attrsOut = nullptr);
